...
from nubespark.streaming.niagara_filodb_config import config
import pyhaystack
import hszinc
import sys
...
def createContextFunction():
    ...
    def get_rng(start=None, tz=None):
    ...
    def get_history_json(point_id, tz, default_tz, start):
        if tz is not None:
            default_tz["name"] = tz
    ...
    points.show()
    tz = get_tz()
    his_json = points.rdd.flatMap(lambda row: get_history_json(row["raw_id"], row["tz"], tz, start))
    his_json.cache()
